...
func (r *Result) OverrideMetadata(metadata misscanTypes.Metadata) {
	r.metadata = metadata
}
...
func (r Result) Metadata() misscanTypes.Metadata {
	return r.metadata
}

func (r Result) Range() misscanTypes.Range {
...
func (r *Result) AbsolutePath(fsRoot string, metadata misscanTypes.Metadata) string {
...
func (r *Result) RelativePathTo(fsRoot, to string, metadata misscanTypes.Metadata) string {
...
type MetadataProvider interface {
	GetMetadata() misscanTypes.Metadata
...
func getMetadataFromSource(source interface{}) misscanTypes.Metadata {
	...
	return metaVal.Interface().(misscanTypes.Metadata)
}
...
func (r *Results) SetSourceAndFilesystem(source string, f fs.FS, logicalSource bool) {
	for i := range *r {
		...
		newrng := misscanTypes.NewRange(rng.GetLocalFilename(), rng.GetStartLine(), rng.GetEndLine(), source, f)
		if logicalSource {
			newrng = misscanTypes.NewRangeWithLogicalSource(rng.GetLocalFilename(), rng.GetStartLine(), rng.GetEndLine(),
				source, f)
		}
		parent := m.Parent()
		switch {
		case m.IsExplicit():
			m = misscanTypes.NewExplicitMetadata(newrng, m.Reference())
		default:
			m = misscanTypes.NewMetadata(newrng, m.Reference())
		}
		if parent != nil {
