...
4`,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			meta := misscanTypes.NewMetadata(
				misscanTypes.NewRange(test.filename, test.startInnerLine, test.endInnerLine, "", fsys),
				"",
			)
			if test.startOuterLine > 0 {
				meta = meta.WithParent(misscanTypes.NewMetadata(
					misscanTypes.NewRange(test.filename, test.startOuterLine, test.endOuterLine, "", fsys),
					"",
				))
			}
			result := &Result{
