...
`,
		...
		{
			name: "rule and a finding on the same line",
			src:  `test #misscan:ignore:rule-1`,
			args: args{
				metadata: metadataWithLine(filename, 1),
				ids:      []string{"rule-1"},
			},
		...
		},
		{
			...
			src:  `test #misscan:ignore:rule-1 #misscan:ignore:rule-2`,
			args: args{
				metadata: metadataWithLine(filename, 1),
				ids:      []string{"rule-2"},
			},
...
`,
		...
		},
		{
			...
			src:  `#misscan:ignore:rule-1[param1=1]`,
			args: args{
				metadata: metadataWithLine(filename, 2),
				ids:      []string{"rule-1"},
			},
		...
		},
		{
			...
			src:  `#misscan:ignore:rule-1:exp:2022-01-01`,
			args: args{
				metadata: metadataWithLine(filename, 2),
				ids:      []string{"rule-1"},
			},
		...
		},
		{
			...
			src:  `#misscan:ignore:rule-1:exp:2026-01-01`,
			args: args{
				metadata: metadataWithLine(filename, 2),
				ids:      []string{"rule-1"},
			},
		...
		},
		{
			...
			src:  `#misscan:ignore:rule-1:exp:2026-99-01`,
			args: args{
				metadata: metadataWithLine(filename, 2),
				ids:      []string{"rule-1"},
			},
...
`,
		...
		{
			name: "multiple ignore rules on the same line",
			src: `# misscan:ignore:rule-1 # misscan:ignore:rule-2
# misscan:ignore:rule-3
test #misscan:ignore:rule-4
`,
			args: args{
				metadata: metadataWithLine(filename, 3),
				ids:      []string{"rule-2"},
...
func TestRules_IgnoreWithCustomIgnorer(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "happy",
			src:  `#misscan:ignore:rule-1:ws:dev`,
		...
		{
			name: "bad",
			src:  `#misscan:ignore:rule-1:ws:prod`,
			parser: &ignore.StringMatchParser{
				SectionKey: "ws",
			},
			args: args{
