...
func Test_RegoScanning_AbsolutePolicyPath_Deny(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tmp, "policies", "test.rego"), []byte(`package misscan.test
...
func Test_RegoScanning_WithRuntimeValues(t *testing.T) {
	...
	_ = os.Setenv("MISSCAN_RUNTIME_VAL", "AOK")
...
deny_evil {
    output := opa.runtime()
	output.env.MISSCAN_RUNTIME_VAL == "AOK"
}
`,
	})
