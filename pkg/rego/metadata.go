...
func (sm *StaticMetadata) populate(meta map[string]any) error {
	...
	upd(&sm.MinimumMisscanVersion, "minimum_misscan_version")
	...
	if raw, ok := meta["minimum_misscan_version"]; ok {
		sm.MinimumMisscanVersion = raw.(string)
	}
...
func (sm *StaticMetadata) ToRule() scan.Rule {
	...
	return scan.Rule{
		...
		MinimumMisscanVersion: sm.MinimumMisscanVersion,
	}
}
...
func parseSelectorItem(raw map[string]any) (Selector, error) {
	...
	if rawType, ok := raw["type"]; ok {
		...
		if selector.Type == string(misscanTypes.SourceDefsec) {
			selector.Type = string(misscanTypes.SourceCloud)
		}
	}
