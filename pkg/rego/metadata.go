			...
			for _, rawSelector := range each {
				...
				if selectorMap, ok := rawSelector.(map[string]interface{}); ok {
					if rawType, ok := selectorMap["type"]; ok {
						...
						// handle backward compatibility for "misscan" source type which is now "cloud"
						if selector.Type == string(misscanTypes.SourceMisscan) {
	...
	for _, policy := range policies {
		for _, annotation := range policy.Annotations {
			for _, ss := range annotation.Schemas {
				...
				if schemaName != "input" {
					if schema, ok := SchemaMap[misscanTypes.Source(schemaName)]; ok {
						customFound = true
