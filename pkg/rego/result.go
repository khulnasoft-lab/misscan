...
func (r regoResult) GetMetadata() misscanTypes.Metadata {
	var m misscanTypes.Metadata
	if !r.Managed {
		m = misscanTypes.NewUnmanagedMetadata()
	} else {
		rng := misscanTypes.NewRangeWithFSKey(r.Filepath, r.StartLine, r.EndLine, r.SourcePrefix, r.FSKey, r.FS)
		if r.Explicit {
			m = misscanTypes.NewExplicitMetadata(rng, r.Resource)
		} else {
			m = misscanTypes.NewMetadata(rng, r.Resource)
		}
	}
	if r.Parent != nil {
...
func (s *Scanner) convertResults(set rego.ResultSet, input Input, namespace string, rule string, traces []string) scan.Results {
	...
	if input.Contents != nil {
		if xx, ok := input.Contents.(map[string]interface{}); ok {
			if md, ok := xx["__misscan_metadata"]; ok {
