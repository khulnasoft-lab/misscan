...
resource "aws_s3_bucket" "test" {}`,
		...
		{
			name: "misscan inline rule ignore all checks",
			source: `resource "aws_s3_bucket" "test" {
    bucket = "" // %s:ignore:*
}`,
			assertLength: 0,
...
resource "aws_s3_bucket" "test" {}`,
		...
		{
			name: "ignore by alias with misscan prefix",
			source: `#%s:ignore:my-alias
resource "aws_s3_bucket" "test" {}`,
			assertLength: 0,
		},
...
`
	...
	tests := []struct {
	...
	}{
		{
			...
			source: `// misscan:ignore:*[ingress.from_port=80]
...
`,
		...
		},
		{
			...
			source: `// misscan:ignore:*[ingress.0.from_port=80]
resource "aws_security_group" "loadbalancer" {
  name = "test"
...
resource "aws_s3_bucket" "test" {}`,
		...
		{
			name: "with expiry and workspace, misscan prefix",
			src: `# misscan:ignore:aws-s3-non-empty-bucket:exp:2221-01-02:ws:testworkspace
resource "aws_s3_bucket" "test" {}`,
			expectedFailed: 0,
		},
		{
			name: "bad workspace, misscan prefix",
			src: `# misscan:ignore:aws-s3-non-empty-bucket:exp:2221-01-02:ws:otherworkspace
resource "aws_s3_bucket" "test" {}`,
			expectedFailed: 1,
		},
		{
