...
func (r *Resource) setFile(target fs.FS, filepath string) {
	r.rng = misscanTypes.NewRange(filepath, r.rng.GetStartLine(), r.rng.GetEndLine(), r.rng.GetSourcePrefix(), target)
...
func (r *Resource) UnmarshalYAML(value *yaml.Node) error {
	r.rng = misscanTypes.NewRange("", value.Line-1, calculateEndLine(value), "", nil)
	r.comment = value.LineComment
	return value.Decode(&r.Inner)
}

func (r *Resource) UnmarshalJSONWithMetadata(node jfather.Node) error {
	r.rng = misscanTypes.NewRange("", node.Range().Start.Line, node.Range().End.Line, "", nil)
