...
func (p *Property) setFileAndParentRange(target fs.FS, filepath string, parentRange misscanTypes.Range) {
	p.rng = misscanTypes.NewRange(filepath, p.rng.GetStartLine(), p.rng.GetEndLine(), p.rng.GetSourcePrefix(), target)
...
func (p *Property) UnmarshalYAML(node *yaml.Node) error {
	p.rng = misscanTypes.NewRange("", node.Line, calculateEndLine(node), "", nil)
...
func (p *Property) UnmarshalJSONWithMetadata(node jfather.Node) error {
	p.rng = misscanTypes.NewRange("", node.Range().Start.Line, node.Range().End.Line, "", nil)
...
func (p *Property) Range() misscanTypes.Range {
	return p.rng
}
...
func (p *Property) Metadata() misscanTypes.Metadata {
	...
	return misscanTypes.NewMetadata(p.Range(), ref.String())
}

func (p *Property) MetadataWithValue(resolvedValue *Property) misscanTypes.Metadata {
