...
func (a *adapter) adaptCluster(apiCluster types.ClusterSummary) (*emr.Cluster, error) {
	...
	if apiCluster.Name != nil {
		name = misscanTypes.String(*apiCluster.Name, metadata)
	}

	releaseLabel := misscanTypes.StringDefault("", metadata)
	if output.Cluster != nil && output.Cluster.ReleaseLabel != nil {
		releaseLabel = misscanTypes.String(*output.Cluster.ReleaseLabel, metadata)
	}

	serviceRole := misscanTypes.StringDefault("", metadata)
	if output.Cluster != nil && output.Cluster.ServiceRole != nil {
		serviceRole = misscanTypes.String(*output.Cluster.ServiceRole, metadata)
	}
...
func (a *adapter) adaptConfig(config types.SecurityConfigurationSummary) (*emr.SecurityConfiguration, error) {
	...
	name := misscanTypes.StringDefault("", metadata)
	if config.Name != nil {
		name = misscanTypes.String(*config.Name, metadata)
	}

	secConf := misscanTypes.StringDefault("", metadata)
	if output.SecurityConfiguration != nil {
		secConf = misscanTypes.String(*output.SecurityConfiguration, metadata)
	}
