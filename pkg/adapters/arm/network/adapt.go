...
func adaptSecurityGroupRule(resource azure.Resource) network.SecurityGroupRule {
	...
	allow := misscanTypes.BoolDefault(false, resource.Metadata)
	if resource.Properties.GetMapValue("access").AsString() == "Allow" {
		allow = misscanTypes.Bool(true, resource.Metadata)
	}

	outbound := misscanTypes.BoolDefault(false, resource.Metadata)
	if resource.Properties.GetMapValue("direction").AsString() == "Outbound" {
		outbound = misscanTypes.Bool(true, resource.Metadata)
	}
