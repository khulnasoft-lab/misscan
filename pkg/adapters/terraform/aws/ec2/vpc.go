...
func adaptVPC(modules terraform.Modules, block *terraform.Block, def bool) ec2.VPC {
	...
	return ec2.VPC{
		...
		ID:              misscanTypes.StringUnresolvable(block.GetMetadata()),
		IsDefault:       misscanTypes.Bool(def, block.GetMetadata()),
		SecurityGroups:  nil,
		FlowLogsEnabled: misscanTypes.BoolDefault(hasFlowLogs, block.GetMetadata()),
	}
}
...
func (a *sgAdapter) adaptSecurityGroups(modules terraform.Modules) []ec2.SecurityGroup {
	...
	for _, resource := range modules.GetResourcesByType("aws_default_security_group") {
		...
		sg.IsDefault = misscanTypes.Bool(true, sg.Metadata)
		sg.Description = misscanTypes.String("", sg.Metadata)
		sg.VPCID = misscanTypes.String("", sg.Metadata)
	...
	if len(orphanResources) > 0 {
		orphanage := ec2.SecurityGroup{
			...
			IngressRules: nil,
			EgressRules:  nil,
			IsDefault:    misscanTypes.BoolUnresolvable(misscanTypes.NewUnmanagedMetadata()),
			VPCID:        misscanTypes.StringUnresolvable(misscanTypes.NewUnmanagedMetadata()),
		}
...
func (a *naclAdapter) adaptNetworkACLs(modules terraform.Modules) []ec2.NetworkACL {
	...
	if len(orphanResources) > 0 {
		orphanage := ec2.NetworkACL{
			Metadata:      misscanTypes.NewUnmanagedMetadata(),
			Rules:         nil,
			IsDefaultRule: misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
		}
...
func (a *sgAdapter) adaptSecurityGroup(resource *terraform.Block, module terraform.Modules) ec2.SecurityGroup {
	...
	return ec2.SecurityGroup{
		...
		IsDefault:    misscanTypes.Bool(false, misscanTypes.NewUnmanagedMetadata()),
...
func adaptSGRule(resource *terraform.Block) ec2.SecurityGroupRule {
	...
	if protocolAddr.IsNumber() {
		protocol = misscanTypes.String(strconv.Itoa(int(protocolAddr.AsNumber())), protocolAddr.GetMetadata())
	}
...
func (a *naclAdapter) adaptNetworkACL(resource *terraform.Block, module *terraform.Module) ec2.NetworkACL {
	...
	return ec2.NetworkACL{
		Metadata:      resource.GetMetadata(),
		Rules:         networkRules,
		IsDefaultRule: misscanTypes.BoolDefault(false, resource.GetMetadata()),
	}
}
...
func adaptNetworkACLRule(resource *terraform.Block) ec2.NetworkACLRule {
	...
	typeVal := misscanTypes.StringDefault("ingress", resource.GetMetadata())

	egressAtrr := resource.GetAttribute("egress")
	if egressAtrr.IsTrue() {
		typeVal = misscanTypes.String("egress", egressAtrr.GetMetadata())
	} else if egressAtrr.IsNotNil() {
		typeVal = misscanTypes.String("ingress", egressAtrr.GetMetadata())
	}
