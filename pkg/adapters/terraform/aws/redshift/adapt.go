...
func adaptCluster(resource *terraform.Block, module *terraform.Module) redshift.Cluster {
	cluster := redshift.Cluster{
		...
		LoggingEnabled:                   misscanTypes.Bool(false, resource.GetMetadata()),
		AutomatedSnapshotRetentionPeriod: misscanTypes.Int(0, resource.GetMetadata()),
		AllowVersionUpgrade:              resource.GetAttribute("allow_version_upgrade").AsBoolValueOrDefault(true, resource),
		VpcId:                            misscanTypes.String("", resource.GetMetadata()),
		...
		SubnetGroupName: misscanTypes.StringDefault("", resource.GetMetadata()),
	}
	...
	if KMSKeyIDAttr.IsResourceBlockReference("aws_kms_key") {
		if kmsKeyBlock, err := module.GetReferencedBlock(KMSKeyIDAttr, resource); err == nil {
			cluster.Encryption.KMSKeyID = misscanTypes.String(kmsKeyBlock.FullName(), kmsKeyBlock.GetMetadata())
		}
	}
