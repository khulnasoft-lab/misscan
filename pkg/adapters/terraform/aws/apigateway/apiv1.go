...
func adaptStageV1(stageBlock *terraform.Block, modules terraform.Modules) v1.Stage {
	stage := v1.Stage{
		...
		AccessLogging: v1.AccessLogging{
			Metadata:              stageBlock.GetMetadata(),
			CloudwatchLogGroupARN: misscanTypes.StringDefault("", stageBlock.GetMetadata()),
		},
	...
	for _, methodSettings := range modules.GetReferencingResources(stageBlock, "aws_api_gateway_method_settings", "stage_name") {
		...
		restMethodSettings := v1.RESTMethodSettings{
			...
			Method:             misscanTypes.String("", methodSettings.GetMetadata()),
			CacheDataEncrypted: misscanTypes.BoolDefault(false, methodSettings.GetMetadata()),
			CacheEnabled:       misscanTypes.BoolDefault(false, methodSettings.GetMetadata()),
		}
	...
	if accessLogging := stageBlock.GetBlock("access_log_settings"); accessLogging.IsNotNil() {
		...
		stage.AccessLogging.CloudwatchLogGroupARN = misscanTypes.StringDefault("", stageBlock.GetMetadata())
	}
