	...
	if len(orphanResources) > 0 {
		orphanage := database.MSSQLServer{
			...
			Server: database.Server{
				Metadata:                  misscanTypes.NewUnmanagedMetadata(),
				EnableSSLEnforcement:      misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
				MinimumTLSVersion:         misscanTypes.StringDefault("", misscanTypes.NewUnmanagedMetadata()),
				EnablePublicNetworkAccess: misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
				FirewallRules:             nil,
			},
	...
	if len(orphanResources) > 0 {
		orphanage := database.PostgreSQLServer{
			...
			Config: database.PostgresSQLConfig{
				Metadata:             misscanTypes.NewUnmanagedMetadata(),
				LogCheckpoints:       misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
				ConnectionThrottling: misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
				LogConnections:       misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
			},
		}
		for _, policy := range orphanResources {
			orphanage.FirewallRules = append(orphanage.FirewallRules, adaptFirewallRule(policy))
...
func (a *mssqlAdapter) adaptMSSQLServer(resource *terraform.Block, module *terraform.Module) database.MSSQLServer {
	minTLSVersionVal := misscanTypes.StringDefault("", resource.GetMetadata())
	publicAccessVal := misscanTypes.BoolDefault(true, resource.GetMetadata())
	enableSSLEnforcementVal := misscanTypes.BoolDefault(false, resource.GetMetadata())
...
func (a *mariaDBAdapter) adaptMariaDBServer(resource *terraform.Block, module *terraform.Module) database.MariaDBServer {
	...
	return database.MariaDBServer{
		...
		Server: database.Server{
			Metadata:                  resource.GetMetadata(),
			EnableSSLEnforcement:      enableSSLEnforcementVal,
			MinimumTLSVersion:         misscanTypes.StringDefault("", resource.GetMetadata()),
			EnablePublicNetworkAccess: publicAccessVal,
			FirewallRules:             firewallRules,
		},
	}
...
func adaptPostgreSQLConfig(resource *terraform.Block, configBlocks []*terraform.Block) database.PostgresSQLConfig {
	config := database.PostgresSQLConfig{
		...
		ConnectionThrottling: misscanTypes.BoolDefault(false, resource.GetMetadata()),
	...
	for _, configBlock := range configBlocks {
		...
		if nameAttr.Equals("log_checkpoints") {
			config.LogCheckpoints = misscanTypes.Bool(valAttr.Equals("on"), valAttr.GetMetadata())
		}
		if nameAttr.Equals("connection_throttling") {
			config.ConnectionThrottling = misscanTypes.Bool(valAttr.Equals("on"), valAttr.GetMetadata())
		}
		if nameAttr.Equals("log_connections") {
			config.LogConnections = misscanTypes.Bool(valAttr.Equals("on"), valAttr.GetMetadata())
		}
	}
