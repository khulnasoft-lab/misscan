...
func NewAttribute(attr *hcl.Attribute, ctx *context.Context, module string, parent misscanTypes.Metadata, parentRef Reference, moduleSource string, moduleFS fs.FS) *Attribute {
	rng := misscanTypes.NewRange(
		attr.Range.Filename,
	...
	metadata := misscanTypes.NewMetadata(rng, reference.String())
	return &Attribute{
		hclAttribute: attr,
		ctx:          ctx,
		module:       module,
...
func (a *Attribute) GetMetadata() misscanTypes.Metadata {
	return a.metadata
}
...
func (a *Attribute) AsBytesValueOrDefault(defaultValue []byte, parent *Block) misscanTypes.BytesValue {
	if a.IsNil() {
		return misscanTypes.BytesDefault(defaultValue, parent.GetMetadata())
	}
	if !a.IsResolvable() || !a.IsString() {
		return misscanTypes.BytesUnresolvable(a.GetMetadata())
	}
	return misscanTypes.BytesExplicit(
		[]byte(a.Value().AsString()),
		a.GetMetadata(),
	)
}

func (a *Attribute) AsStringValueOrDefault(defaultValue string, parent *Block) misscanTypes.StringValue {
	if a.IsNil() {
		return misscanTypes.StringDefault(defaultValue, parent.GetMetadata())
	}
	if !a.IsResolvable() || !a.IsString() {
		return misscanTypes.StringUnresolvable(a.GetMetadata())
	}
	return misscanTypes.StringExplicit(
		a.Value().AsString(),
		a.GetMetadata(),
	)
}
...
func (a *Attribute) AsBoolValueOrDefault(defaultValue bool, parent *Block) misscanTypes.BoolValue {
	if a.IsNil() {
		return misscanTypes.BoolDefault(defaultValue, parent.GetMetadata())
	}
	if !a.IsResolvable() || !a.IsBool() {
		return misscanTypes.BoolUnresolvable(a.GetMetadata())
	}
	return misscanTypes.BoolExplicit(
		a.IsTrue(),
		a.GetMetadata(),
	)
}

func (a *Attribute) AsIntValueOrDefault(defaultValue int, parent *Block) misscanTypes.IntValue {
	if a.IsNil() {
		return misscanTypes.IntDefault(defaultValue, parent.GetMetadata())
	}
	if !a.IsResolvable() || !a.IsNumber() {
		return misscanTypes.IntUnresolvable(a.GetMetadata())
	}
	flt := a.AsNumber()
	return misscanTypes.IntExplicit(
		int(flt),
		a.GetMetadata(),
	)
}
...
func (a *Attribute) AsStringValues() misscanTypes.StringValueList {
	if a == nil {
		return nil
	}
...
// nolint
func (a *Attribute) getStringValues(expr hcl.Expression, ctx *hcl.EvalContext) (results []misscanTypes.StringValue) {

	defer func() {
		if err := recover(); err != nil {
			results = []misscanTypes.StringValue{misscanTypes.StringUnresolvable(a.metadata)}
		}
	}()
	...
	case *hclsyntax.TupleConsExpr:
		for _, expr := range t.Exprs {
			...
			if err != nil {
				results = append(results, misscanTypes.StringUnresolvable(a.metadata))
				continue
			}
	...
	case *hclsyntax.FunctionCallExpr, *hclsyntax.ConditionalExpr:
		...
		if err != nil {
			return append(results, misscanTypes.StringUnresolvable(a.metadata))
		}
...
func (a *Attribute) valueToStrings(value cty.Value) (results []misscanTypes.StringValue) {
	defer func() {
		if err := recover(); err != nil {
			results = []misscanTypes.StringValue{misscanTypes.StringUnresolvable(a.metadata)}
		}
	...
	if value.IsNull() {
		return []misscanTypes.StringValue{misscanTypes.StringUnresolvable(a.metadata)}
	}
	if !value.IsKnown() {
		return []misscanTypes.StringValue{misscanTypes.StringUnresolvable(a.metadata)}
	}
...
func (a *Attribute) valueToString(value cty.Value) (result misscanTypes.StringValue) {
	defer func() {
		if err := recover(); err != nil {
			result = misscanTypes.StringUnresolvable(a.metadata)
		}
	}()
	...
	switch value.Type() {
	case cty.String:
		return misscanTypes.String(value.AsString(), a.metadata)
	default:
		return result
	}
}
...
func (a *Attribute) AsMapValue() misscanTypes.MapValue {
	return safeOp(a, func(v cty.Value) misscanTypes.MapValue {
		if !a.IsMapOrObject() {
