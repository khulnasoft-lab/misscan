...
func AssertMisscanEqual(t *testing.T, expected interface{}, actual interface{}) {
	...
	if expectedJson[0] == '[' {
		...
		assert.Equal(t, expectedSlice, actualSlice, "misscan adapted and expected values do not match")
	} else {
		...
		assert.Equal(t, expectedMap, actualMap, "misscan adapted and expected values do not match")
	}
}
