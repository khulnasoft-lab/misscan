	...
	for _, r := range instanceResources {
		instance := ec2.Instance{
			...
			MetadataOptions: ec2.MetadataOptions{
				Metadata:     r.Metadata(),
				HttpTokens:   misscanTypes.StringDefault("optional", r.Metadata()),
				HttpEndpoint: misscanTypes.StringDefault("enabled", r.Metadata()),
			},
...
func getBlockDevices(r *parser.Resource) []*ec2.BlockDevice {
	...
	for _, d := range devicesProp.AsList() {
		...
		var result misscanTypes.BoolValue
		if encrypted.IsNil() {
			result = misscanTypes.BoolDefault(false, d.Metadata())
		} else {
