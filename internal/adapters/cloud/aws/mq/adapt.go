...
func (a *adapter) adaptBroker(apiBroker types.BrokerSummary) (*mq.Broker, error) {
	...
	return &mq.Broker{
		Metadata:     metadata,
		PublicAccess: misscanTypes.Bool(output.PubliclyAccessible, metadata),
		Logging: mq.Logging{
			Metadata: metadata,
			General:  misscanTypes.Bool(output.Logs != nil && output.Logs.General, metadata),
			Audit:    misscanTypes.Bool(output.Logs != nil && output.Logs.Audit, metadata),
		},
	}, nil
}
