...
func (a *adapter) getMFADevices(user iamtypes.User) ([]iam.MFADevice, error) {
	...
	for _, apiDevice := range apiDevices {
		...
		devices = append(devices, iam.MFADevice{
			Metadata:  metadata,
			IsVirtual: misscanTypes.Bool(isVirtual, metadata),
		})
	}
...
func (a *adapter) getUserKeys(apiUser iamtypes.User) ([]iam.AccessKey, error) {
	...
	for {
		...
		for _, apiAccessKey := range output.AccessKeyMetadata {
			...
			lastUsed := misscanTypes.TimeUnresolvable(metadata)
			if output, err := a.api.GetAccessKeyLastUsed(a.Context(), &iamapi.GetAccessKeyLastUsedInput{
				...
				if output.AccessKeyLastUsed != nil && output.AccessKeyLastUsed.LastUsedDate != nil {
					lastUsed = misscanTypes.Time(*output.AccessKeyLastUsed.LastUsedDate, metadata)
				}
			}

			accessKeyId := misscanTypes.StringDefault("", metadata)
			if apiAccessKey.AccessKeyId != nil {
				accessKeyId = misscanTypes.String(*apiAccessKey.AccessKeyId, metadata)
			}

			creationDate := misscanTypes.TimeDefault(time.Now(), metadata)
			if apiAccessKey.CreateDate != nil {
				creationDate = misscanTypes.Time(*apiAccessKey.CreateDate, metadata)
			}

			keys = append(keys, iam.AccessKey{
				Metadata:     metadata,
				AccessKeyId:  accessKeyId,
				Active:       misscanTypes.Bool(apiAccessKey.Status == iamtypes.StatusTypeActive, metadata),
...
func (a *adapter) adaptUser(apiUser iamtypes.User) (*iam.User, error) {
	...
	lastAccess := misscanTypes.TimeUnresolvable(metadata)
	if apiUser.PasswordLastUsed != nil {
		lastAccess = misscanTypes.Time(*apiUser.PasswordLastUsed, metadata)
	}

	username := misscanTypes.StringDefault("", metadata)
	if apiUser.UserName != nil {
		username = misscanTypes.String(*apiUser.UserName, metadata)
	}
