...
func (a *adapter) adaptDistribution(distribution types.DistributionSummary) (*cloudfront.Distribution, error) {
	...
	for _, cacheBehaviour := range config.DistributionConfig.CacheBehaviors.Items {
		cacheBehaviours = append(cacheBehaviours, cloudfront.CacheBehaviour{
			Metadata:             metadata,
			ViewerProtocolPolicy: misscanTypes.String(string(cacheBehaviour.ViewerProtocolPolicy), metadata),
		})
	}
	...
	return &cloudfront.Distribution{
		Metadata: metadata,
		WAFID:    misscanTypes.String(wafID, metadata),
		Logging: cloudfront.Logging{
			Metadata: metadata,
			Bucket:   misscanTypes.String(loggingBucket, metadata),
		},
		DefaultCacheBehaviour: cloudfront.CacheBehaviour{
			Metadata:             metadata,
			ViewerProtocolPolicy: misscanTypes.String(defaultCacheBehaviour, metadata),
		},
		OrdererCacheBehaviours: cacheBehaviours,
		ViewerCertificate: cloudfront.ViewerCertificate{
			Metadata:               metadata,
			MinimumProtocolVersion: misscanTypes.String(minimumProtocolVersion, metadata),
		},
	}, nil
}
