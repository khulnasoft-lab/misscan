...
func (a *adapter) adaptLaunchTemplate(template types.LaunchTemplate) (*ec2.LaunchTemplate, error) {
	...
	if templateData.MetadataOptions != nil {
		instance.MetadataOptions.HttpTokens = misscanTypes.StringDefault(string(templateData.MetadataOptions.HttpTokens), metadata)
		instance.MetadataOptions.HttpEndpoint = misscanTypes.StringDefault(string(templateData.MetadataOptions.HttpEndpoint), metadata)
	}
	...
	if templateData.BlockDeviceMappings != nil {
		for _, blockMapping := range templateData.BlockDeviceMappings {
			...
			if blockMapping.Ebs != nil && blockMapping.Ebs.Encrypted != nil {
				ebsDevice.Encrypted = misscanTypes.BoolDefault(*blockMapping.Ebs.Encrypted, metadata)
			}
