...
func getClusters(modules terraform.Modules) (clusters []rds.Cluster) {
	...
	if len(orphanResources) > 0 {
		orphanage := rds.Cluster{
			...
			BackupRetentionPeriodDays: misscanTypes.IntDefault(1, misscanTypes.NewUnmanagedMetadata()),
			ReplicationSourceARN:      misscanTypes.StringDefault("", misscanTypes.NewUnmanagedMetadata()),
			...
			Encryption: rds.Encryption{
				Metadata:       misscanTypes.NewUnmanagedMetadata(),
				EncryptStorage: misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
				KMSKeyID:       misscanTypes.StringDefault("", misscanTypes.NewUnmanagedMetadata()),
			},
			PublicAccess:         misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
			Engine:               misscanTypes.StringUnresolvable(misscanTypes.NewUnmanagedMetadata()),
			LatestRestorableTime: misscanTypes.TimeUnresolvable(misscanTypes.NewUnmanagedMetadata()),
			DeletionProtection:   misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
			SkipFinalSnapshot:    misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
		}
...
func adaptClusterInstance(resource *terraform.Block, modules terraform.Modules) rds.ClusterInstance {
	...
	if clusterIdAttr.IsResourceBlockReference("aws_rds_cluster") {
		if referenced, err := modules.GetReferencedBlock(clusterIdAttr, resource); err == nil {
			clusterId = misscanTypes.String(referenced.FullName(), referenced.GetMetadata())
		}
	}
...
func adaptInstance(resource *terraform.Block, modules terraform.Modules) rds.Instance {
	...
	return rds.Instance{
		...
		ReplicationSourceARN:             misscanTypes.StringExplicit(replicaSourceValue, resource.GetMetadata()),
		...
		LatestRestorableTime:             misscanTypes.TimeUnresolvable(resource.GetMetadata()),
...
func adaptDBParameterGroups(resource *terraform.Block, modules terraform.Modules) rds.ParameterGroups {
	...
	for _, paramres := range paramres {
		...
		Parameters = append(Parameters, rds.Parameters{
			...
			ParameterName:  misscanTypes.StringDefault("", paramres.GetMetadata()),
			ParameterValue: misscanTypes.StringDefault("", paramres.GetMetadata()),
		})
	}
...
func adaptCluster(resource *terraform.Block, modules terraform.Modules) (rds.Cluster, []string) {
	...
	return rds.Cluster{
		...
		Encryption:                adaptEncryption(resource),
		PublicAccess:              misscanTypes.Bool(public, resource.GetMetadata()),
		Engine:                    resource.GetAttribute("engine").AsStringValueOrDefault(rds.EngineAurora, resource),
		LatestRestorableTime:      misscanTypes.TimeUnresolvable(resource.GetMetadata()),
