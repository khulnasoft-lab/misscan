...
func adaptNetworks(modules terraform.Modules) (networks []compute.Network) {
	...
	for _, subnetworkBlock := range modules.GetResourcesByType("google_compute_subnetwork") {
		...
		if logConfigBlock := subnetworkBlock.GetBlock("log_config"); logConfigBlock.IsNotNil() {
			subnetwork.EnableFlowLogs = misscanTypes.BoolExplicit(true, subnetworkBlock.GetBlock("log_config").GetMetadata())
		}
...
func expandRange(ports string, attr *terraform.Attribute) []misscanTypes.IntValue {
	...
	if !strings.Contains(ports, "-") {
		...
		return []misscanTypes.IntValue{
			misscanTypes.Int(i, attr.GetMetadata()),
		}
	}
	...
	var output []misscanTypes.IntValue
	for i := start; i <= end; i++ {
		output = append(output, misscanTypes.Int(i, attr.GetMetadata()))
	}
	return output
}
...
func adaptFirewallRule(firewall *compute.Firewall, firewallBlock, ruleBlock *terraform.Block, allow bool) {
	...
	var ports []misscanTypes.IntValue
