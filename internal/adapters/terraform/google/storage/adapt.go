...
func (a *adapter) adaptBuckets() []storage.Bucket {
	...
	orphanage := storage.Bucket{
		...
		Location:                       misscanTypes.StringDefault("", misscanTypes.NewUnmanagedMetadata()),
		EnableUniformBucketLevelAccess: misscanTypes.BoolDefault(false, misscanTypes.NewUnmanagedMetadata()),
		Members:                        nil,
		Bindings:                       nil,
	}
...
func (a *adapter) adaptBucketResource(resourceBlock *terraform.Block) storage.Bucket {
	...
	bucket := storage.Bucket{
		...
		Encryption: storage.BucketEncryption{
			Metadata:          resourceBlock.GetMetadata(),
			DefaultKMSKeyName: misscanTypes.StringDefault("", resourceBlock.GetMetadata()),
		},
	}
